...
namespace DFS
{
  ...
  public:
     ...
     */
    bool send_raw_data(int, const void *,
                       std::uint32_t) const;
     ...
     * @param deserialization_function Deserializes the buffer and stores them in a node_info object vector.
     */
    void connect_clients(DFS_Interface::NodeInfoVector &, bool &,
                         const std::function<DFS_Interface::Node_Info(int)> &);

    /**
     * @brief Handles the response received from the clients.
     * @param execute_info Reference to the Execute_Info object to store the execution information.
     ...
     */
    bool handle_response(DFS_Interface::Execute_Info &, int &,
  ...
  private:
    ...
    std::vector<int> client_sock;                /**< Vector of client socket file descriptors. */
    ...
    int max_sd, act_;                            /**< Variables for tracking the maximum file descriptor and activity. */
